...
if(backup.format!=='curator-error-bus-kv-recovery')fail('Unexpected backup format.');
...
console.log('Error Bus recovery backup is valid.');
console.log(`Exported: ${backup.exportedAt||'unknown'}`);
console.log(`KV keys: ${backup.data.entries.length}`);
console.log(`SHA-256: ${actual}`);

function fail(message){console.error(`Invalid Error Bus recovery backup: ${message}`);process.exit(1);}
