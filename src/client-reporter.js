import base from './quiet-client-recovery.js';
...
async function verifyGet(url){try{const response=await nativeFetch(url,{method:'GET',cache:'no-store',credentials:'same-origin'});return !!(response&&response.ok);}catch{return false}}
window.addEventListener('error',async e=>{
  if(e.target&&e.target!==window){
    ...
    send({kind:'resource-error',component:'frontend-resource',message:tag+' failed to load',resource:clean(src)});
    return;
  }
  send({kind:'javascript-error',component:'frontend-runtime',message:clean(e.message||'JavaScript error'),filename:clean(e.filename),line:e.lineno||null,column:e.colno||null,stack:clean(e.error&&e.error.stack,4000)});
},true);
...
if(nativeFetch){window.fetch=async function(input,init){
  try{
    ...
    let u='';try{u=new URL(typeof input==='string'?input:input.url,location.href).href}catch{}
    const opaque=response&&(response.status===0||response.type==='opaque'||response.type==='opaqueredirect');
    if(!opaque&&!response.ok&&u&&!u.startsWith(BUS)){
      send({kind:'fetch-http-error',component:'frontend-network',message:'Fetch returned HTTP '+response.status,resource:clean(u),status:response.status,method:clean((init&&init.method)||'GET',12)});
    }
    return response;
  }catch(error){
    let u='';try{u=new URL(typeof input==='string'?input:input.url,location.href).href}catch{}
    if(!u||u.startsWith(BUS))throw error;
    const method=clean((init&&init.method)||'GET',12).toUpperCase();
    if(method==='GET'&&samePageOrigin(u)&&await verifyGet(u))throw error;
    send({kind:'fetch-network-error',component:'frontend-network',message:clean(error&&error.message||'Fetch failed'),resource:clean(u),method,stack:clean(error&&error.stack,4000)});
    throw error;
  }
}}
