...
function renderConsole(incidents) {
  ...
  const card = incident => {
    ...
    return `<article class="incident ${escapeHtml(incident.severity)}"><header><strong>${escapeHtml(String(incident.severity || '').toUpperCase())}</strong><span>${escapeHtml(incident.status)}</span></header><h2>${escapeHtml(incident.source)}</h2><p class="component">${escapeHtml(incident.component)} · ${escapeHtml(incident.type)}</p><p>${escapeHtml(incident.message)}</p>${where}${actions}<small>First ${escapeHtml(incident.firstSeenAt || '')} · Last ${escapeHtml(incident.lastSeenAt || '')} · ${Number(incident.occurrences || 1)} occurrence${Number(incident.occurrences || 1) === 1 ? '' : 's'}</small></article>`;
  };
